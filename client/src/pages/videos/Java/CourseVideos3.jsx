...
const videos = [
  { title: 'Introduction to Java Programming', duration: '7 min', type: 'Video' },
  { title: 'What is Java ?', duration: '3 min', type: 'Video' },
  { title: 'What is Java Full Stack?', duration: '10 min', type: 'Video' },
  { title: 'Frontend Development', duration: '17 min', type: 'Video' },
  { title: 'Backend Development', duration: '22 min', type: 'Video' },
];
  ...
  return (
    <section className="flex flex-col items-start max-w-full w-[94%]">
      ...
      {videos.slice(2).map((video, index) => (
        <Link to={'/java/videosplay3'}><div className='w-full'><VideoItem key={index + 2} {...video} isFirst={false} /></div></Link>
      ))}
    </section>
  );
}
